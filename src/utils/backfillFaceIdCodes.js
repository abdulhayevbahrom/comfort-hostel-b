...
export async function backfillFaceIdCodes() {
  const students = await Student.find({ $or: [{ faceIdCode: { $exists: false } }, { faceIdCode: null }, { faceIdCode: '' }, { faceIdCode: /^STU-[A-F0-9]{24}$/ }] }).select('_id')
  ...
  const employees = await Employee.find({ $or: [{ faceIdCode: { $exists: false } }, { faceIdCode: null }, { faceIdCode: '' }, { faceIdCode: /^EMP-[A-F0-9]{24}$/ }] }).select('_id')
