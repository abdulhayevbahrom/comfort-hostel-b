...
const faceAccessEventSchema = new mongoose.Schema({
  ...
  decision: {
    type: String,
    enum: ['granted', 'granted_warning', 'observed_unknown', 'denied_unknown', 'denied_inactive', 'denied_disabled', 'denied_debt_limit', 'error'],
    required: true,
    index: true,
  },
  ...
  smsStatus: { type: String, enum: ['not_required', 'queued', 'sending', 'sent', 'failed', 'duplicate', 'limit_reached'], default: 'not_required', index: true },
